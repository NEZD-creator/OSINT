...
export function setupIpLogger(bot: Bot) {
  bot.command("iplogger", async (ctx) => {
    ...
    // URL backend appleta
    const baseUrl = process.env.APP_URL || "https://ais-dev-ksm5jkv5h246kcdmj7zddl-783580602421.europe-west2.run.app"; // Fallback if no specific route 
    const trackLink = `${baseUrl}/api/l/${chatId}`;
